...
- **Storefront static sahifalar (5) ixchamlashtirildi** (`about`, `terms`, `privacy`, `faq`, `returns`):
  ...
  - **Qo'llangan o'lchamlar**: Hero/header `py-16/py-20` → `py-8/py-10`, `text-4xl..8xl` → `text-2xl..5xl`, `mt-3 mb-6` → `mt-2 mb-4`; container `max-w-4xl/5xl p-8..16` → `max-w-3xl p-5/6`, `-mt-12/-mt-10` → `-mt-6`; card `rounded-3xl/rounded-[40px]/[2.5rem]/[3.5rem]` → `rounded-2xl`, `p-8..12` → `p-4..6`; FAQ item `p-6` → `p-4`, `text-lg` → `text-base`, icon `w-5 h-5` → `w-4 h-4`; section spacing `space-y-6..12` → `space-y-4..6`, `mb-8..20` → `mb-4..8`; text `text-lg/text-xl` → `text-sm/text-base` (`leading-relaxed` saqlandi); list `py-3/py-4` → `py-2/py-1`, `gap-3/gap-4` → `gap-2/gap-3`; button `px-8 py-4 rounded-xl` → `px-5 py-2.5 rounded-lg`; grid gap `gap-6..12` → `gap-3/gap-4`; about timeline `space-y-12/16` → `space-y-8/10`, node `w-10 h-10` → `w-8 h-8`, year `text-5xl` → `text-2xl/3xl`, team avatar `w-32/40` → `w-24/28`.
  - **O'zgartirilgan fayllar (5)**: `src/app/(root)/[locale]/{about,terms,privacy,faq,returns}/page.tsx`.
  - **Saqlangan**: Kontent va i18n key'lar o'zgarmadi. Ranglar, gradient'lar, layout, `md:`/`lg:` breakpoint'lar saqlandi. Funksionallik saqlandi — FAQ `<details>` accordion (8), terms sidebar nav / prev-next / `window.print()` / privacy link / print CSS (print-only bo'limlar o'zgarmadi), returns check/x ro'yxatlar, framer-motion/AnimatePresence.
  - Verification: `npx tsc --noEmit` 0 error, ESLint o'zgargan 5 fayl 0 error, `npm run build` SUCCESS (101/101 static, Node 22), runtime 15 route (`/uz,/ru,/en` × about/terms/privacy/faq/returns) HTTP 200.
...
- **Favicon/SEO/branding complete fix (Chrome favicon kichik + Google globe + eski contactlar)**:
  ...
  - **Yangi assetlar**: `src/app/favicon.ico` (16/32/48 multi-size ICO, hand-rolled ICO container + PNG payloads), `public/favicon-16x16.png`, `public/favicon-32x32.png`, `public/icon-192.png`, `public/icon-512.png` (transparent, wordmark), `public/icon-maskable-192.png`/`512.png` (PWA maskable, white tile), `public/apple-touch-icon.png` (180x180 white tile), `public/og-image.png` (regenerate, 1200x630 brand OG).
  - **Conflict fix**: `public/favicon.ico` o'chirildi — `src/app/favicon.ico` Next.js convention'iga shadow bo'lardi. Bitta authoritative source faqat `src/app/favicon.ico`.
  - **Manifest**: `public/manifest.json` — name "HADAF Market", short_name "HADAF", theme_color `#4361ee`, maskable icons qo'shildi.
  - **Eski contact'lar tozalandi** (DB StoreSettings id='default' source of truth): `admin/invoices/add` va `[id]/edit` billFrom'dan `+998 76 200 01 05` / `info@hadaf.uz` / "Termez, Surxondaryo" olib tashlandi (bo'sh default), `scripts/auth-bot.mjs` `uzm.uz` → `www.alhadaf.uz`, `scripts/seed-delivery.ts` `At-Termiziy ko'chasi` → `Surxondaryo viloyati Termiz Shahri Alisher Navoiy ko'chasi`, settings/stores/orders placeholder'lari generic qilindi.
  - **Meta**: `uz/ru/en.json` Meta.description — "eng katta online bozor" (asossiz da'vo) → customer-focused HADAF description (Google spam risk).
  - **JSON-LD**: `src/app/(root)/[locale]/layout.tsx` — Organization'ga DB'dagi real instagram (`hadaf.market.uz`) va youtube (`hadaf_market_uz`) profillari `sameAs` sifatida qo'shildi (faqat brand nomiga mos keladiganlar; Telegram linki Qalam_Books_rasmiy — HADAF emas, tashlab ketilgan; supportTelegram shaxsiy username — qo'shilmaydi).
  - Verification: `npx tsc --noEmit` 0 error, ESLint o'zgargan fayllar 0 error (1 pre-existing warning CreateOrderForm), `npm run build` SUCCESS (Node 22). (Fayllar: `scripts/generate-icons.mjs`, `public/manifest.json`, `messages/{uz,ru,en}.json`, `src/app/(root)/[locale]/layout.tsx`, invoice add/edit, auth-bot.mjs, seed-delivery.ts, settings/stores/orders placeholders)
...
- **Category Attribute Definition — Parent-chain inheritance + Standard Product Template seed**:
  ...
  - **Route o'zgarishlari**: `GET /api/admin/categories/[id]/attributes` endi effective defs qaytaradi (`_count` values saqlanadi). `GET/PUT /api/admin/products/[id]/attributes` endi product category'si + parent chain defs'ni qaytaradi (product child kategoriyada bo'lsa root defs ham qaytadi; bir nechta category bo'lsa name bo'yicha dedupe). POST/PATCH yaratish o'zgarmadi — local defs faqat exact category'ga yoziladi. Product Builder frontend (`UniversalProductSections.tsx`) o'zgarmadi — backend'dan effective defs oladi.
  - **Seed script** (`scripts/seed-product-templates.mjs`, yangi, idempotent): real kategoriyalarga standart template defs qo'shadi. Qalambooks root: `format` (SELECT Yumshoq muqova/Qattiq muqova/Fleksibl, forVariant=true, order 7) — mavjud 7 defs saqlanadi. Kiyim-kechaklar: `brend` (TEXT), `mavsum` (SELECT Bahor/Yoz/Kuz/Qish), `rang` (COLOR, forVariant=true) qo'shildi; `material`/`o_lcham` forVariant=true saqlandi (faqat order 1..5 ketma-ketligiga moslandi). O'yinchoqlar: `yosh_toifasi` (SELECT 0-3,4-7,8-12,13+), `material` (TEXT), `brend` (TEXT), `rang` (COLOR, forVariant=true), `olcham` (SELECT Kichik/O'rta/Katta). Bolalar kiyimlari alohida seed'lanmadi (inheritance). `name` bo'yicha existing tekshiruvi — duplicate yaratmaydi; existing defs/content/produktlarga tegmaydi (faqat Order spec bo'yicha `order` qiymatini moslashtiradi). 2-marta ishga tushirilganda 0 duplicate (verified).
  - DB schema o'zgarmadi, yangi migration YO'Q. Existing productlar/attribute qiymatlari buzilmadi.
  - Verification: `npx tsc --noEmit` 0 error, ESLint 0, `npm run build` SUCCESS (101/101 static, Node 22). Real HTTP (minted admin JWT): Badiiy adabiyotlar (Qalambooks child) → 8 defs (inherited), Kiyim → 5 defs, O'yinchoq → 5 defs, Bolalar kiyimlari → 5 defs (2-level chain), Xitoy root → 0. Override (child same-name g'alaba) + cycle-safe unit test PASS. (Fayllar: `src/lib/category-definitions.ts` (yangi), `src/app/api/admin/categories/[id]/attributes/route.ts`, `src/app/api/admin/products/[id]/attributes/route.ts`, `scripts/seed-product-templates.mjs` (yangi))
