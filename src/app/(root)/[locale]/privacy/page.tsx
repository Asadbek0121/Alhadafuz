...
export default function PrivacyPage() {
    ...
    return (
        <div className="bg-[#f8fafc] min-h-screen pb-20">
            {/* Hero Section */}
            <div className="bg-gradient-to-br from-[#0052FF] via-[#0033CC] to-[#001A66] py-8 text-center text-white relative overflow-hidden">
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-bold uppercase tracking-wider mb-4">
                        <ShieldCheck size={16} className="text-blue-300" />
                        {t('badge')}
                    </div>
                    <h1 className="text-2xl md:text-3xl font-black tracking-tight mb-2">{t('title')}</h1>
                    <p className="text-blue-100 text-sm md:text-base font-medium max-w-2xl mx-auto leading-relaxed">
                        {t('subtitle')}
                    </p>
                </motion.div>
            </div>
            ...
            <div className="container max-w-3xl -mt-6 relative z-10 px-4">
                <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 p-5 md:p-6 border border-blue-50">

                    {/* Key Highlights */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">
                        {highlights.map((item, i) => (
                            <div key={i} className={`p-4 rounded-2xl flex flex-col items-center text-center group hover:bg-white hover:shadow-xl transition-all duration-500`}
                                style={{ backgroundColor: item.color === 'blue' ? '#eff6ff' : item.color === 'emerald' ? '#ecfdf5' : '#eef2ff' }}
                            >
                                <div className={`w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-blue-600 mb-2 group-hover:bg-blue-600 group-hover:text-white transition-colors`}>
                                    <item.icon size={20} />
                                </div>
                    ...
                    <div className="prose prose-slate max-w-none prose-headings:text-slate-900 prose-headings:font-black prose-p:text-slate-600 prose-p:leading-relaxed prose-strong:text-slate-900 prose-li:text-slate-600">
                        <div className="space-y-6">
                            <section>
                                <h2 className="text-lg flex items-center gap-3 mb-4">
                            ...
                            <section>
                                ...
                                <ul className="list-none space-y-2 pl-0">
                                    {purposes.map((text, idx) => (
                                        <li key={idx} className="flex gap-3 items-start p-3 rounded-xl bg-white border border-slate-100 shadow-sm">
                                            <div className="w-5 h-5 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{idx + 1}</div>
                            ...
                            <section className="p-5 rounded-2xl bg-slate-900 text-white shadow-xl relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
                                <h2 className="text-lg flex items-center gap-3 mb-4 text-white">
                            ...
                            <section>
                                ...
                                <div className="space-y-2">
                                    <div className="flex gap-3 items-start border-l-4 border-blue-600 pl-4 py-1">
                                        <p className="text-sm m-0">{t('s5_i1')}</p>
                                    </div>
                                    <div className="flex gap-3 items-start border-l-4 border-slate-300 pl-4 py-1">
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {userRights.map((text, i) => (
                                        <div key={i} className="flex items-center gap-2 p-3 rounded-xl bg-blue-50/50 border border-blue-100/50">
                    ...
                    <div className="mt-6 p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 text-white text-center relative overflow-hidden border border-white/10">
                        <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
                        <div className="relative z-10 space-y-3">
                            <div className="inline-flex p-3 rounded-full bg-blue-600/20 text-blue-400 mb-1">
                                <ShieldCheck size={20} />
                            </div>
                            <h3 className="text-base md:text-lg font-black">{t('cta_title')}</h3>
                            <p className="text-slate-400 max-w-xl mx-auto text-sm leading-relaxed">
                                {t('cta_desc')}
                            </p>
                            <div className="flex flex-col md:flex-row items-center justify-center gap-3 mt-4">
                                <a href="mailto:privacy@hadaf.uz" className="flex items-center gap-2 bg-[#0052FF] text-white px-5 py-2.5 rounded-lg font-black text-sm hover:bg-[#0040CC] transition-all hover:scale-105 shadow-xl shadow-blue-600/20 uppercase tracking-widest">
                                    <Bell size={16} /> privacy@hadaf.uz
                                </a>
                                <Link href={`/${locale}/terms`} className="flex items-center gap-2 bg-white/10 backdrop-blur-md text-white px-5 py-2.5 rounded-lg font-black text-sm hover:bg-white/20 transition-all border border-white/10 uppercase tracking-widest">
                                    {t('cta_btn_terms')}
                                </Link>
                            </div>
                        </div>
