...
14. **Variantli mahsulotlar**: Phase C admin UI + Phase D attribute definitions UI tayyor. Qalambooks kategoriyasiga 7 ta definition insert qilingan (forVariant=false). Lekin hali forVariant=true definition yo'q (masalan Rang/O'lcham) va real variantli mahsulotlar yaratilmagan. Structured attribute saqlash cheklovi: category o'zgartirilsa va joriy defs'ga kirmaydigan structured qiymatlar saqlanganda `PUT .../attributes` (bulk deleteMany+createMany) ularni o'chiradi (legacy `Product.attributes` JSON saqlanadi).
...
2026-08-21 (Universal Product Admin UI — forVariant Xususiyat/Variant UX fix)
