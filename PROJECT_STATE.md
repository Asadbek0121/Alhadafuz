...
- **Category Attribute Definition — Parent-chain inheritance + Standard Product Template seed**:
  ...
  - **Seed script** `scripts/seed-product-templates.mjs` (idempotent): Qalambooks root → `format` (SELECT, forVariant=true) qo'shildi (order 7, mavjud 7 defs saqlanadi). Kiyim-kechaklar → `brend` (TEXT), `mavsum` (SELECT: Bahor/Yoz/Kuz/Qish), `rang` (COLOR, forVariant=true) qo'shildi; `material` (TEXT, forVariant=true) va `o_lcham` (SELECT) saqlandi — faqat order 1,2,3,4,5 ketma-ketligiga moslandi. O'yinchoqlar → 5 defs: `yosh_toifasi` (SELECT: 0-3,4-7,8-12,13+), `material` (TEXT), `brend` (TEXT), `rang` (COLOR, forVariant=true), `olcham` (SELECT: Kichik/O'rta/Katta). Bolalar kiyimlari alohida seed'lanmadi (Kiyim-kechaklar'dan inheritance). 2-marta ishga tushirilganda 0 duplicate (verified).
  - DB schema o'zgarmadi (migration yo'q). Existing productlar/qiymatlar buzilmadi (public `/api/products/[id]` legacy attributes + attributeValues/variants saqlanadi, product page 200).
  - Verification: tsc 0, ESLint 0, `next build` SUCCESS (101/101 static, Node 22). Real HTTP test: minted admin JWT session → Badiiy adabiyotlar 8 defs (root'dan inherited), Kiyim 5 defs, O'yinchoq 5 defs, Bolalar kiyimlari 5 defs (2-level chain), Xitoy root 0. Override + cycle-safe unit test PASS (temp kategoriyalar bilan, cleanup qilindi).
- **Storefront Universal Product Variant integration**: Product detail'da haqiqiy variant selector (Rang/O'lcham — variantKey/variantLabel dan parse), variant tanlanganda price/stock/SKU/compareAtPrice/gallery (VariantImage) o'zgarishi, addToCart'ga `variantId`/`sku`/`variant` snapshot yuborish, addToCart guard (variant tanlanmasa blok), `CartItem` interface'ga `variantId`/`sku`, `cartItemKey` variantId-aware, Cart/CartDrawer/Checkout'da variant label + SKU ko'rsatish, Orders API'ga `variantId`/`variantSnapshot`/`sku`/`barcode`/`attributesSnapshot` snapshot saqlash (server authoritative price + variant-aware stock decrement), `cart/sync` variant-aware merge (productId+variantId). CHINA_ORDER bilan birga ishlaydi. Variantsiz (kitob) oqim buzilmadi. Yangi modul: `src/lib/variant-utils.ts`. DB schema o'zgarmadi. tsc 0 / lint 0 / build SUCCESS (101/101, Node 22).
- **Universal Product Admin UI — forVariant Xususiyat/Variant UX fix**: `AttributeFields` endi faqat `forVariant=false` defs oladi (`attributeDefs`), `VariantEditor` faqat `forVariant=true` (`variantDefs`) — Material kabi definition IKKI joyda chiqishi tuzatildi (root cause: AttributeFields'ga hamma defs uzatilardi). Validation faqat Xususiyatlar bo'limidagi defs uchun; save'da majburiy forVariant defs qiymati variant o'qidan derive qilinadi (backend o'zgarmadi). UX: "Xususiyatlar"/"Variantlar" sarlavha+helper, "Sotib olish variantlari" (o'qlar), kitob holatida "Variantlar — Bu mahsulot uchun variantlar belgilanmagan." + generator tugmasi yo'q, 1 kombinatsiya tushuntirishi, kartezian breakdown ("6 (2 × 3 = 6)"). (`UniversalProductSections.tsx`, `VariantEditor.tsx`) tsc 0 / lint 0 / build SUCCESS (101/101, Node 22).
- **Phase D — Category Attribute Definition Admin UI**: Yangi `/admin/categories/[id]/attributes` sahifasi (CRUD). `DefinitionForm.tsx` — 8 attribute type (TEXT/NUMBER/BOOLEAN/SELECT/MULTI_SELECT/COLOR/MEASUREMENT/DATE) uchun dinamik forma (Type'ga qarab Options/Unit/Min/Max ko'rsatiladi), frontend `validateDefinitionTypeCombo` validatsiyasi + backend authoritative (409 duplicate name ko'rsatiladi). Definition list — order bo'yicha, type/required/variant badge'lar, options/unit/min/max meta, Edit (PATCH) / Delete (DELETE). `/admin/categories/[id]` → `/admin/categories/[id]/attributes` ga `redirect()`. **Link fix**: `UniversalProductSections.tsx:499,535` dagi `/admin/categories/${categoryId}` (404) → `/admin/categories/${categoryId}/attributes`. Kategoriya ro'yxat sahifasiga (tree+list) "Xususiyatlarni boshqarish" (⚙️) tugmasi qo'shildi. **Test data**: Qalambooks root kategoriyasi (`cmo7j45hj0001ce6n7qnkascd`) uchun 7 ta real definition DB'ga insert qilindi (Muallif, Nashriyot, Til SELECT, Sahifa soni NUMBER min1/max10000, Muqova SELECT, ISBN, Nashr yili NUMBER) — forVariant=false. DB schema o'zgarmadi (yangi migration yo'q). (`src/app/(admin)/admin/categories/[id]/`, `src/components/admin/product/UniversalProductSections.tsx`, `src/app/(admin)/admin/categories/page.tsx`)
...
- **Vercel build P1002 fix (4)**: `prepare-direct-url.mjs` endi `prisma migrate deploy` ni `execSync` orqali o'z process'ida ishga tushiradi — `process.env.DIRECT_URL` (true direct host) child process'ga meros beriladi. `.env` fayliga yozish yetarli emas edi (Vercel env var' override qiladi). (`scripts/prepare-direct-url.mjs`, `package.json`)
